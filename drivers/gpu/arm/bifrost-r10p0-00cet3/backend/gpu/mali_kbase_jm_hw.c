				...
				int js)
{
	...
	for (slot_nr = 0; slot_nr < kbdev->gpu_props.num_job_slots; slot_nr++) {
		struct kbase_jd_atom * tmp_atom1 = NULL;
		struct kbase_jd_atom * tmp_atom2 = NULL;

		if (slot_nr == js)
			continue;

		tmp_atom1 = kbase_gpu_inspect(kbdev, slot_nr, 0);
		tmp_atom2 = kbase_gpu_inspect(kbdev, slot_nr, 1);

		if ((tmp_atom1 && (kctx != tmp_atom1->kctx))||(tmp_atom2 && (kctx != tmp_atom2->kctx)))
